...
/**
 * This class is the program performing Meteorite search.
 * The program is interactive.
 * When the program is executed the name of the input CSV file containing the list of all the Meteorites and it's data
 * as the program's single command line argument. The data in this CSV file serves as a database of all the Meteorites.
 *
 * In the interactive part:
 * - if the user enters location LATITUDE LONGITUDE, The program responds by printing the nearest Meteorite.
 * - if the user enters year YEAR, The program responds by printing a list of Meteorites with that year.
 * - if the user enters mass MASS, The program responds by printing a list of Meteorites with that mass +/-10.
 *
 * @author Brendan Tang
 * @version 10/7/2020
 */
public class FallenStars {
  ...
  /**
	 ...
	 * the first string should be the name of the input csv file containing the list of Meteorites.
	 */
  public static void main(String[] args) {
    ...
    // Adds Meteorites to MeteoriteList
    while(in.hasNextLine()){
      ...
      // String instructions = "Search the database by using one of the following queries."
          ...
          if (!userValue.equalsIgnoreCase("quit")) {
            ...
            try{
              ...
              if(part[0].equals("location") && part.length == 3){
                ...
                Meteorite res = list.getByLocation(loc);
                output = res.toString();
              }

              // User enters: year YEAR
              else if(part[0].equals("year") && part.length == 2){
                MeteoriteLinkedList mll = list.getByYear(Integer.parseInt(part[1]));
                output = mll.toString();
              }

              // User enters: mass MASS
              else if(part[0].equals("mass") && part.length == 2){
                MeteoriteLinkedList mll = list.getByMass(Integer.parseInt(part[1]), 10);
                output = mll.toString();
              }

              // User enters anything else
              else{
                // Purposely cause ArrayIndexOutOfBoundsException Error if no location, year, or mass argument
